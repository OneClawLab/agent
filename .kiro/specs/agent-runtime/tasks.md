...
- [ ] 6. 出站投递模块
  - [ ] 6.1 实现 `src/runner/deliver.ts` 出站投递逻辑
...
- [ ] 8. Checkpoint - 确保 runner 模块测试通过
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 9. CLI 命令实现
  - [ ] 9.1 实现 `src/commands/init.ts`
  ...
  - [ ] 9.5 实现 `src/commands/deliver.ts` 出站投递命令
    - 调用 runner/deliver.ts 的 deliverBatch
    - _Requirements: 7.1, 7.6_
  - [ ] 9.6 实现 `src/commands/status.ts` 和 `src/commands/list.ts`
